...
class TicTacToe:
    ...
    # Check if there a win (it test every possibility) (Return True and print winner if Win, Return False if not)
    def win(self):
        for i in range(len(self.grille)):
    ...
    # Check if there Draw (Return true if draw, Return false if not)
    def draw(self):
        tiret = 0
        for i in range(len(self.grille)):
